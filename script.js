...
function renderFilters(){const filters=[['all','الكل'],['dry','للبشرة الجافة'],['oily','للبشرة الدهنية'],['scrub','تقشير']];$('#filters').innerHTML=filters.map(([id,label])=>`<button class="filter-btn ${state.filter===id?'active':''}" data-filter="${id}">${label}</button>`).join('')}
function renderProducts(){const list=state.filter==='all'?products:products.filter(p=>p.category===state.filter|| (state.filter==='dry'&&p.id===3)); $('#productGrid').innerHTML=list.map(p=>`<article class="card"><img class="product-image" src="${p.image}" alt="${p.name}"><div class="card-body"><div class="badge-row"><span class="tag badge">${p.badge}</span><span class="tag best">${p.tag}</span></div><h3 class="product-title">${p.name}</h3><p class="product-desc">${p.description}</p><div class="quick-points">${p.highlights.map(h=>`<span>${h}</span>`).join('')}</div><div class="meta"><div><small>أنسب استخدام</small><strong>${p.bestFor}</strong></div><div class="price">${money(p.price)}</div></div><div class="card-actions"><button class="btn btn-primary add-to-cart" data-id="${p.id}">أضف إلى السلة</button><button class="btn btn-ghost view-product" data-id="${p.id}">عرض التفاصيل</button></div></div></article>`).join('')}
function renderTestimonials(){$('#testimonialsGrid').innerHTML=testimonials.map(t=>`<article class="testimonial"><div class="stars">★★★★★</div><div class="mini">🧼 ${t.product}</div><p>"${t.text}"</p><strong>${t.name}</strong><div class="mini">${t.location}</div></article>`).join('')}
...
function addToCart(id){const p=products.find(x=>x.id===id);const found=state.cart.find(i=>i.id===id);if(found){found.qty+=1}else{state.cart.push({...p,qty:1})} saveCart(); openCart()}
function updateCartUI(){const totalCount=state.cart.reduce((a,b)=>a+b.qty,0); $('#cartCount').textContent=totalCount; const itemsBox=$('#cartItems'); const footer=$('#cartFooter'); if(!state.cart.length){itemsBox.innerHTML='<div class="empty">السلة فارغة. أضيفي بعض المنتجات أولًا.</div>'; footer.innerHTML=''; return} itemsBox.innerHTML=state.cart.map(item=>`<div class="cart-item"><img src="${item.image}" alt="${item.name}"><div><h4>${item.name}</h4><p>${item.weight}</p><p>${money(item.price)}</p><div class="qty-row"><button data-action="inc" data-id="${item.id}">+</button><strong>${item.qty}</strong><button data-action="dec" data-id="${item.id}">-</button></div></div><button class="remove-btn" data-action="remove" data-id="${item.id}">🗑️</button></div>`).join(''); const total=state.cart.reduce((s,i)=>s+i.price*i.qty,0); footer.innerHTML=`<div class="cart-summary"><div class="cart-summary-row"><span>المجموع</span><strong class="price">${money(total)}</strong></div><p class="helper">سيتم تجهيز رسالة واتساب تلقائيًا تحتوي على تفاصيل الطلب، ثم نؤكد معك الشحن والعنوان.</p><button class="btn btn-whatsapp" id="checkoutBtn">إرسال الطلب عبر واتساب</button></div>`}
function checkout(){const total=state.cart.reduce((s,i)=>s+i.price*i.qty,0);const lines=state.cart.map((i,n)=>`${n+1}. ${i.name}\nالكمية: ${i.qty}\nالسعر: ${money(i.price*i.qty)}`).join('\n\n'); const msg=`مرحبًا، أريد إتمام طلب المنتجات التالية:\n\n${lines}\n\nالمجموع: ${money(total)}\nالشحن: يتم تأكيده حسب المنطقة`; window.open(`https://wa.me/201095314011?text=${encodeURIComponent(msg)}`,'_blank')}
function openProduct(id){const p=products.find(x=>x.id===id); $('#productModalContent').innerHTML=`<div class="modal-layout"><img src="${p.image}" alt="${p.name}"><div class="modal-info"><span class="tag badge">${p.badge}</span><h3 id="modalTitle">${p.name}</h3><div class="price">${money(p.price)}</div><p>${p.longDescription}</p><div class="quick-points">${p.highlights.map(h=>`<span>${h}</span>`).join('')}</div><h4>الفوائد</h4><ul class="modal-list">${p.benefits.map(b=>`<li>${b}</li>`).join('')}</ul><h4>المكونات</h4><ul class="modal-list">${p.ingredients.map(b=>`<li>${b}</li>`).join('')}</ul><h4>طريقة الاستخدام</h4><p>${p.usage}</p><div class="hero-actions"><button class="btn btn-primary modal-add" data-id="${p.id}">أضف إلى السلة</button><a class="btn btn-whatsapp" target="_blank" rel="noopener" href="https://wa.me/201095314011">اسأل على واتساب</a></div></div></div>`; $('#productModalOverlay').classList.remove('hidden'); document.body.style.overflow='hidden'}
function closeProduct(){ $('#productModalOverlay').classList.add('hidden'); if(!$('#cartOverlay').classList.contains('hidden')) return; document.body.style.overflow='' }
...
document.addEventListener('click',e=>{
  const filter=e.target.closest('.filter-btn'); if(filter){state.filter=filter.dataset.filter; renderFilters(); renderProducts()}
  const add=e.target.closest('.add-to-cart,.modal-add'); if(add){addToCart(Number(add.dataset.id)); if(add.classList.contains('modal-add')) closeProduct()}
  ...
  if(e.target.id==='cartToggle') openCart(); if(e.target.id==='closeCart' || e.target.id==='cartOverlay') closeCart();
  if(e.target.id==='checkoutBtn') checkout();
  const qa=e.target.closest('.faq-q'); if(qa){qa.parentElement.classList.toggle('open')}
  const action=e.target.dataset.action; if(action){const id=Number(e.target.dataset.id); const item=state.cart.find(i=>i.id===id); if(!item)return; if(action==='inc') item.qty+=1; if(action==='dec'){if(item.qty>1)item.qty-=1; else state.cart=state.cart.filter(i=>i.id!==id)} if(action==='remove') state.cart=state.cart.filter(i=>i.id!==id); saveCart()}
});
...
renderFilters(); renderProducts(); renderTestimonials(); renderFeatures(); renderSteps(); renderShipping(); renderFaq(); updateCartUI();

...
const faqs = [
  ...
  { q: 'هل الشموع مناسبة للهدايا؟', a: 'نعم، كثير من منتجات SIRAJ مناسبة كهدايا أنيقة أو لمسات ديكور بسيطة ومميزة.' },
...
function getVisibleProducts() {
  ...
  return byFilter
    ...
    .filter((product) => state.priceMax === null || Number(product.price) <= state.priceMax);
}
...
function renderProducts() {
  const el = $('#productGrid');
  /* expose for quick chips */
  window.__alzahraaState = state;
  window.alzahraaRenderProducts = renderProducts;
...
function openProduct(id) {
  ...
  try {
    ...
    // Refresh the RV section (it only shows after at least 1 view)
...
function renderRecentlyViewed() {
  ...
  let ids;
  try {
    ids = JSON.parse(localStorage.getItem('candles-rv') || '[]');
  } catch { ids = []; }

  if (!ids.length) { section.hidden = true; return; }

  const products = ids
    .map(id => currentProducts.find(p => Number(p.id) === Number(id)))
    .filter(Boolean)
    .slice(0, 4);
  ...
  track.innerHTML = products.map(p => `
    <article class="rv-card" data-id="${p.id}" tabindex="0" role="button"
      ...
      <div class="rv-body">
        <p class="rv-name">${escHtml(p.name)}</p>
        <span class="rv-price">${money(p.price)}</span>
      </div>
    </article>
  `).join('');

  // Tap to open product sheet

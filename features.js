/* ================================================================
   ...
   1. Wishlist / Favorites  (heart on every card, localStorage)
   2. Search Autocomplete   (live dropdown while typing)
   3. Scent Finder Quiz     (3-step quiz → filtered results)
...
(function () {
  ...
  function injectQuizTrigger() {
    ...
    btn.type = 'button';
    btn.innerHTML = '✨ ساعديني أختار الشمعة المناسبة';
  ...
  function init() {
    ...
    const guideWatcher = new MutationObserver(() => {
      if (document.getElementById('guide')) {
        injectQuizTrigger();
        guideWatcher.disconnect();
      }
    });
    guideWatcher.observe(document.body, { childList: true, subtree: true });
    injectQuizTrigger(); // try immediately too

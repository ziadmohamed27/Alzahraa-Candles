...
(function () {
  ...
  function injectModalQty() {
    ...
    const mo = new MutationObserver(() => {
      ...
      if (ctaFooter) {
        ctaFooter.parentNode.insertBefore(wrap, ctaFooter);
      } else {
        const addBtnEl = modal.querySelector('.modal-add');
  ...
  function init() {
    ...
    if (prodGrid) {
      // Disconnect after first real render — don't keep observing forever
      const rvObs = new MutationObserver(() => {
        if (prodGrid.querySelector('.skeleton-card') || !prodGrid.children.length) return;
        renderRecentlyViewed();
        rvObs.disconnect(); // one-shot: render once when products load
      });
